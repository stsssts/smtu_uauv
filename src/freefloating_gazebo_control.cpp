...
#include "freefloating_gazebo_control.h"
                                     ...
                                     sdf::ElementPtr _sdf)
{
  ...
  if (control_body_)
  {
      ...
      if (sdf_element->GetName() == "thruster")
      {
        ...
        else if (sdf_element->HasElement("name"))
        {
          ...
          thruster_links_.push_back(
              model_->GetLink(sdf_element->Get<std::string>("name")));
    ...
    thruster_use_publisher_ =
        rosnode_.advertise<sensor_msgs::JointState>("thruster_use", 1);

    ros::SubscribeOptions ops;
    ops = ros::SubscribeOptions::create<sensor_msgs::JointState>(
        body_command_topic, 1,
        boost::bind(&FreeFloatingControlPlugin::ThrusterCommandCallBack, this,
                    _1),
  ...
  if (control_joints_ && model_->GetJointCount() != 0)
  {
    ...
    ros::SubscribeOptions ops =
        ros::SubscribeOptions::create<sensor_msgs::JointState>(
            ...
            boost::bind(&FreeFloatingControlPlugin::JointCommandCallBack, this,
                        _1),
            ros::VoidPtr(), &callback_queue_);
    ...
    joint_state_publisher_ =
        rosnode_.advertise<sensor_msgs::JointState>(joint_state_topic, 1);
  ...
  // switch_service_ = rosnode_.advertiseService<std_srvs::EmptyRequest,
  // std_srvs::EmptyResponse>("switch",
  // &FreeFloatingControlPlugin::SwitchService, this);
...
void FreeFloatingControlPlugin::Update()
{
  ...
  if (controller_is_running_)
  {
    ...
    if (control_body_ && body_command_received_ &&
        (body_->GetWorldCoGPose().pos.z < z_surface_))
    {
      // saturate thruster use
      double norm_ratio = 1;
      unsigned int i;
      for (i = 0; i < thruster_fixed_idx_.size(); ++i)
        norm_ratio = std::max(norm_ratio, std::abs(thruster_command_(i)) /
      ...
      for (i = 0; i < thruster_command_.size(); ++i)
        thruster_use_.position[i] =
            100 * std::abs(thruster_command_(i) / thruster_max_command_[i]);
    ...
    const sensor_msgs::JointStateConstPtr& _msg)
{
  ...
  {
    ROS_WARN("Received inconsistent thruster command, name and effort "
             "dimension do not match");
    return;
  }
  else if (!read_effort && (_msg->name.size() != _msg->position.size()))
  {
    ROS_WARN("Received inconsistent thruster command, name and position "
             "dimension do not match");
    return;
  }
